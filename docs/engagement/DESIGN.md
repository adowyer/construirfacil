...
4. ✅ **HECHO (ruteo) — Workflow n8n `CF Engagement Router`** (id `6OkOnL6ROx9n2kH2`, **Active**): webhook `cf-engagement` + JOIN a `marcas` (nodo "Get marca.plan", credencial Supabase API server-side) + ruteo por plan. **Las 4 ramas validadas end-to-end (2026-07-17):** A #6812/#6813 (OTP real), B #6814 (Hausind→cf_ximia, UI real), C #6815 (marca test cf), D #6816 + #6817 (banner "Quiero que me contacten" real, sin marca). Los 4 nodos "Seg …" siguen siendo **NoOp placeholders** → falta reemplazarlos por los envíos reales (mails, alerta asesora, precalif). Verificación sin browser: API de n8n (`GET /api/v1/executions`) con `N8N_API_KEY` de `~/Projects/XIMIA/.env`.
   - ⚠️ **Producción (Vercel) todavía NO emite:** `N8N_ENGAGEMENT_WEBHOOK_URL` sólo está en `.env.local` (local). Falta setearla en Vercel (scope Production) + **redeploy**. Sin eso, el emisor hace no-op silencioso en el sitio real.
   - **Path real del Seg D:** NO es `/cotizar` (ruta huérfana, sólo `CotizadorUber` la enlaza y no está montado en ninguna página → probablemente sólo bots/spam). El path vivo es el CTA **"Quiero que me contacten"** del promo banner → abre `ReservarModal` **genérico** (sin `marca_id`) → Seg D.
   - **Bug aparte (flujo OTP, NO engagement):** en la prueba del banner, `requestOTP` se disparó **3x en 2s** (matchea el "pide OTP dos veces" ya anotado en `ReservarModal`) y el modal quedó colgado en "VERIFICANDO…" sin mostrar el error de código incorrecto. Corresponde a la sesión del catálogo/gate.
